...
*/
  ...
  consumerHelper.SetAttribute ("LifeTime", StringValue (consperc)); //uso temporario da variavel consperc para lifetime
  //consumerHelper.SetAttribute ("Window", StringValue ("5"));

  uint32_t nprefixint =   boost::lexical_cast<uint32_t> (nprefix);
  //uint32_t ttimeint =   boost::lexical_cast<uint32_t> (timesim);
  if (prodxint == 1)
  {
