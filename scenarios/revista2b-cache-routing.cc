...
main (int argc, char *argv[])
{
  ...
  for (NodeContainer::iterator it = consumers.begin(); it != consumers.end(); it++)
  {
	  for (uint32_t i = 0; i < nprefixint; i++)
	  {
		...
		std::string batch = boost::lexical_cast<std::string> (j * timesim / (consumers.GetN()) + "s 100");
		//consumerHelper.SetAttribute ("Batches", StringValue ("1s 1 2s 5 10s 2"));
		consumerHelper.SetAttribute ("Batches", StringValue (batch));
  ...
  for (uint32_t i = 0; i < nApplications; i++)
  {
    ...
    Ptr<ns3::ndn::ConsumerBatches> pc = DynamicCast<ns3::ndn::ConsumerBatches> (p);
    if (pc->GetId () > 0 && fapp)
    {
      confile << pc->GetId ();
      fapp = false;
