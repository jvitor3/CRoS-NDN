...
main (int argc, char *argv[])
{
  ...
  while (i < napp)
  {
    ...
    {
	  appP.Get(i % napp)->SetStartTime (Seconds (constime * i + 0.1));
	  appR.Get(i % napp)->SetStartTime (Seconds (constime * i + 0.1));
	  if (i != napp - 1)
	  {
		  appP.Get(i % napp)->SetStopTime (Seconds (constime * (i + 1)+0.2));	
