...
configureCRoSNDN (std::string prefix, std::string nprefix,
  NodeContainer &nodes,   std::string hellorate,
  NodeContainer &controllers, NodeContainer &producers, NodeContainer &consumers,
  std::string regrate, std::string fibsize, std::string cachesize)
{
  // Install CCNx stack on all nodes
  ndn::StackHelper ccnxHelper;
  ccnxHelper.SetForwardingStrategy ("ns3::ndn::fw::CRoSNDNTunnelStrategy", "MaxFibSize", fibsize);
  ccnxHelper.SetContentStore ("ns3::ndn::cs::Lru", "MaxSize", cachesize);
  ccnxHelper.InstallAll ();
  ...
  // NodeIDReply
  ndn::AppHelper NodeIDReplyHelper ("ns3::ndn::CRoSNDNTunnelHelloProducer");
  //NodeIDReplyHelper.SetAttribute ("Prefix", StringValue("/hello"));
  NodeIDReplyHelper.SetAttribute ("Frequency", StringValue (hellorate));
  NodeIDReplyHelper.Install (nodes);
  
  ndn::AppHelper TunnelInHelper ("ns3::ndn::TunnelIn");
  TunnelInHelper.Install (nodes);
  ndn::AppHelper TunnelOutHelper ("ns3::ndn::TunnelOut");
  TunnelOutHelper.Install (nodes);  
   ...
   std::string regrate, std::string fibsize, std::string cachesize)
{
  if (strategy == "CRoS-NDN")
    configureCRoSNDN (prefix, nprefix, nodes, hellorate, controllers, producers, consumers, regrate, fibsize, cachesize);
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue("consperc", "Cache Size", consperc);  
  ...
  */
  ...
  //  hellorate, controllers, producers, consumers, regrate, fibsize, consperc);
  std::string cachesize = consperc;
  configureCRoSNDN (prefix, nprefix, nodes, hellorate, controllers, producers, consumers, regrate, fibsize, cachesize);

  // Simulated traffic
  // Consumer
  ApplicationContainer app;
  ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerCbrPause");
  ...
  ndn::AppHelper consumerRegisterContentHelper ("ns3::ndn::CRoSNDNRegisterContentConsumer");
  consumerRegisterContentHelper.SetAttribute("Prefix", StringValue(aux.get (0).toBlob ()));
  consumerRegisterContentHelper.SetAttribute("NPrefixes", IntegerValue(boost::lexical_cast<uint32_t>(nprefix)));
  consumerRegisterContentHelper.SetAttribute("RegRate", IntegerValue(boost::lexical_cast<uint32_t>(regrate)));
  consumerRegisterContentHelper.SetAttribute ("Frequency", StringValue (hellorate));
  ...
  for (NodeContainer::iterator it = consumers.begin(); it != consumers.end(); it++)
  {
	  for (uint32_t i = 0; i < nprefixint; i++)
	  {
		...
		consumerHelper.SetAttribute ("PauseTime", 
			StringValue (boost::lexical_cast<std::string>((j+1) * timesim1 / (consumers.GetN()))+"s"));
		ApplicationContainer app1 = consumerHelper.Install (*it);
		app1.Start (Seconds (j * timesim1 / (consumers.GetN())));
		//app1.Stop (Seconds ((j+1) * timesim1 / (consumers.GetN())));
		app.Add (app1);	
		
		//app1.Stop (Seconds ((j+1) * timesim1 / (consumers.GetN())));
		producerHelper.SetAttribute("Prefix", StringValue(aux1.toUri ()));
		ApplicationContainer appcache = producerHelper.Install (*it);
		appcache.Start(Seconds (((j+1) * timesim1 / (consumers.GetN()))));		
	  }
	  j++;
	  //ApplicationContainer appcons = consumerRegisterContentHelper.Install (*it);
	  //appcons.Start(Seconds ((j * timesim1 / (consumers.GetN()))));
	  //j++;
//	  ApplicationContainer appcache = producerHelper.Install (*it);
//	  appcache.Start(Seconds ((j * timesim1 / (consumers.GetN()))));
  }
  ...
  ndn::AppDelayTracer::InstallAll ("./temp/"+suffix+"/app-delays-trace-"+tracepostfix);
  ...
  for (uint32_t i = 0; i < nApplications; i++)
  {
    ...
    Ptr<ns3::ndn::ConsumerCbrPause> pc = DynamicCast<ns3::ndn::ConsumerCbrPause> (p);
    if (pc->GetId () > 0 && fapp)
    {
      confile << pc->GetId ();
      fapp = false;
