...
main (int argc, char *argv[])
{
  ...
  ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerZipfMandelbrotB");
  ...
  for (uint32_t i = 0; i < nApplications; i++)
  {
    ...
    Ptr<ns3::ndn::ConsumerZipfMandelbrotB> pc = DynamicCast<ns3::ndn::ConsumerZipfMandelbrotB> (p);
    if (pc->GetId () > 0 && fapp)
    {
      confile << pc->GetId ();
      fapp = false;
