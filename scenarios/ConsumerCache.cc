  ...
  std::string regrate, std::string fibsize)
{
  ...
  ccnxHelper.InstallAll();
  /*
  NodeContainer notconsumers;
  for (NodeContainer::iterator it = nodes.begin(); it != nodes.end(); it++)
  {
	  bool found = false;
	  for (NodeContainer::iterator it2 = consumers.begin(); it2 != consumers.end(); it2++)
	  {
		  if ((*it) == (*it2))
		  {
			  found = true;
  ...
  ndn::StackHelper ccnxHelper2;
  ccnxHelper2.SetForwardingStrategy ("ns3::ndn::fw::CRoSNDNStrategy", "MaxFibSize", fibsize);
  //ccnxHelper2.SetContentStore ("ns3::ndn::cs::Nocache");
  ccnxHelper2.SetContentStore ("ns3::ndn::cs::Lru", "MaxSize", "5");
  ccnxHelper2.Install (notconsumers);
  */
...
main (int argc, char *argv[])
{
  ...
  */
  ...
  for (NodeContainer::iterator it = consumers.begin(); it != consumers.end(); it++)
  {
      ...
      //app.Start (Seconds (j * timesim / (consumers.GetN())));	  
	  j++;
	  ApplicationContainer appcons = consumerRegisterContentHelper.Install (*it);
	  appcons.Start(Seconds ((j * timesim / (consumers.GetN()))));
	  //j++;
	  //app.Stop(Seconds ((j * timesim / (consumers.GetN()))));
	  ApplicationContainer appcache = producerHelper.Install (*it);
	  appcache.Start(Seconds ((j * timesim / (consumers.GetN()))));
  }
