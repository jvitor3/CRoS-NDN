   ...
   std::string regrate, std::string fibsize)
{
  if (strategy == "CRoS-NDN" || strategy == "CRoS-NDN-CRP" || strategy == "CRoS-NDN-CFP")
    ...
    configureNLSRLike (prefix, nprefix, nodes, hellorate, controllers, producers, consumers, regrate, fibsize);       
...
main (int argc, char *argv[])
{
  ...
  {
    ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerCbrRand2");
  ...
  for (uint32_t i = 0; i < nApplications; i++)
  {
    ...
    {
      Ptr<ns3::ndn::ConsumerCbrRand2> pc = DynamicCast<ns3::ndn::ConsumerCbrRand2> (p);
      if (pc->GetFaceId () > 0 && fapp)
      {
        confile << pc->GetFaceId ();
        fapp = false;
