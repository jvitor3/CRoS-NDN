...
main (int argc, char *argv[])
{
  ...
  if (prodxint == 0)
    comp1 = nodes.size ();
  else if (prodxint >= 1 and prodxint <= nodes.size ())
    comp1 = prodxint;
  
  //for (uint32_t j = 0; j < boost::lexical_cast<uint32_t>(consprefs); j++)
  //{
    for (uint32_t i = 0; i < comp1; i++)
    {
      ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerCbrRand");
      ndn::Name prefix = ndn::Name ("/");
      prefix.appendSeqNum (i);
      //prefix.appendSeqNum (j);      
      consumerHelper.SetAttribute("Prefix", StringValue(prefix.toUri ()));
      ...
      app.Add (consumerHelper.Install (consumers)); // first node
