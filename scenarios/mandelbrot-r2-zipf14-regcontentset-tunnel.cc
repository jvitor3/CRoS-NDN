...
configureOSPFLike (std::string prefix, std::string nprefix, 
  std::string hellorate, NodeContainer &producers, NodeContainer &nodes, std::string fibsize)
{
  ns3::ndn::StackHelper2 ccnxHelper;
  //ccnxHelper.SetForwardingStrategy ("ns3::ndn::fw::FloodingPrefix");
  ccnxHelper.SetForwardingStrategy ("ns3::ndn::fw::OSPFLikeStrategy", "MaxFibSize", fibsize);
  ccnxHelper.SetDefaultRoutes(true);
  ...
  std::string regrate, std::string fibsize)
{
  ...
  //ccnxHelper.SetContentStore ("ns3::ndn::cs::Freshness::Fifo", "MaxSize", "10");
  ...
  for (NodeContainer::Iterator i = producers.Begin (); i != producers.End (); ++i)
  {
    aux = ndn::Name("/");
    aux.appendSeqNum((*i)->GetId ());
    ndn::AppHelper consumerRegisterContentHelper ("ns3::ndn::CRoSNDNRegisterContentSetConsumer");
    consumerRegisterContentHelper.SetAttribute("Prefix", StringValue(aux.get (0).toBlob ()));
    consumerRegisterContentHelper.SetAttribute("NPrefixes",
      IntegerValue(boost::lexical_cast<uint32_t>(nprefix)));
    consumerRegisterContentHelper.SetAttribute("RegRate",
   ...
   std::string regrate, std::string fibsize)
{
  if (strategy == "CRoS-NDN")
    ...
    configureOSPFLike (prefix, nprefix, hellorate, producers, nodes, fibsize);
...
main (int argc, char *argv[])
{
  ...
  ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerZipfMandelbrotPrefix");
  ...
  consumerHelper.SetAttribute ("NumberOfContents", StringValue (consprefs));
  ...
  for (NodeContainer::Iterator i = producers.Begin (); i != producers.End (); ++i)
  {
	...
	producerHelper.SetAttribute ("KeyLocator", StringValue("/foo"));//test
    producerHelper.Install (*i);
  }
  
  for (NodeContainer::Iterator i = consumers.Begin (); i != consumers.End (); ++i)
  {
	consumerHelper.SetAttribute("Prefix", StringValue(aux.toUri ()));
  ...
  for (uint32_t i = 0; i < nApplications; i++)
  {
    ...
    Ptr<ns3::ndn::ConsumerZipfMandelbrotPrefix> pc = DynamicCast<ns3::ndn::ConsumerZipfMandelbrotPrefix> (p);
    if (pc->GetId () > 0 && fapp)
    {
      confile << pc->GetId ();
      fapp = false;
