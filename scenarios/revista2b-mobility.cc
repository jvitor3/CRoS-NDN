  ...
  std::string hellorate, NodeContainer &producers, NodeContainer &nodes, std::string fibsize)
{
  ...
  for (NodeContainer::Iterator i = producers.Begin (); i != producers.End (); ++i)
  {
	...
	appProd.Start(Seconds (((j) * timesim1 / (producers.GetN()))));
	j++;
	appProd.Stop(Seconds (((j) * timesim1 / (producers.GetN()))));
  }  
}
  ...
  std::string regrate, std::string fibsize)
{
  ...
  for (NodeContainer::Iterator i = producers.Begin (); i != producers.End (); ++i)
  {
	...
	consumerRegisterContentHelper.SetAttribute("NewLocation", IntegerValue(1));
    ApplicationContainer app5 = consumerRegisterContentHelper.Install (*i); 
	app5.Start(Seconds (((j) * timesim1 / (producers.GetN()))));
	j++;
	//app5.Stop(Seconds (((j) * timesim1 / (producers.GetN()))));
  }
...
main (int argc, char *argv[])
{
  ...
  for (NodeContainer::Iterator i = producers.Begin (); i != producers.End (); ++i)
  {
    ...
    //aux.appendSeqNum((*i)->GetId ());
	aux.append ("gta");
    ndn::AppHelper producerHelper ("ns3::ndn::ProducerPause");
    producerHelper.SetAttribute("Prefix", StringValue(aux.toUri ()));
    producerHelper.SetAttribute ("PayloadSize", StringValue("1024"));
	producerHelper.SetAttribute ("PauseTime", 
			StringValue (boost::lexical_cast<std::string>((j+1) * timesim1 / (producers.GetN()))+"s"));
	ApplicationContainer appprod = producerHelper.Install (*i);
	appprod.Start(Seconds (((j) * timesim1 / (producers.GetN()))));
	j++;
	//appprod.Stop(Seconds (((j) * timesim1 / (producers.GetN()))));
  }
