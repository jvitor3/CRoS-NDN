...
*/
  ...
  ndn::AppHelper consumerHelper ("ns3::ndn::ConsumerWindow1");
  ...
  consumerHelper.SetAttribute ("MaxSeq", StringValue (boost::lexical_cast<std::string> (std::numeric_limits<uint32_t>::max ())));
  ...
  if (prodxint == 1)
  {
	...
	aux1.appendSeqNum(boost::lexical_cast<uint64_t>(nprefix));
    consumerHelper.SetAttribute("Prefix", StringValue(aux1.toUri ()));
  }
  else
  {
    std::cout << "TODO: more than 1 producer or consumer!";
    return 0;
  }
  ...
  for (uint32_t i = 0; i < nApplications; i++)
  {
    ...
    //if (prodxint == 1)
    {
      Ptr<ns3::ndn::ConsumerWindow1> pc = DynamicCast<ns3::ndn::ConsumerWindow1> (p);
      if (pc->GetId () > 0 && fapp)
      {
        confile << pc->GetId ();
        fapp = false;
    ...
    /*else
    {
      Ptr<ns3::ndn::ConsumerCbrRand4> pc = DynamicCast<ns3::ndn::ConsumerCbrRand4> (p);
