...
void
configureCRoSNDNold (std::string prefix, std::string nprefix,
  NodeContainer &nodes,   std::string hellorate,
  NodeContainer &controllers, NodeContainer &producers, NodeContainer &consumers,
  std::string regrate, std::string fibsize)
{
...
configureCRoSNDN (std::string prefix, std::string nprefix,
  NodeContainer &nodes,   std::string hellorate,
  NodeContainer &controllers, NodeContainer &producers, NodeContainer &consumers, NodeContainer &namenode,
  std::string regrate, std::string fibsize)
{
  // Install CCNx stack on all nodes
  ndn::StackHelper ccnxHelper;
...
//  ccnxHelper.SetContentStore ("ns3::ndn::cs::Freshness::Fifo", "MaxSize", "1000");
  ...
  for (NodeContainer::Iterator i = producers.Begin (); i != producers.End (); ++i)
  {
    aux = ndn::Name("/");
    aux.appendSeqNum((*i)->GetId ());
    ndn::AppHelper consumerRegisterContentHelper ("ns3::ndn::CRoSNDNRegisterContentConsumerExt");
    consumerRegisterContentHelper.SetAttribute("Prefix", StringValue(aux.get (0).toBlob ()));
    consumerRegisterContentHelper.SetAttribute("NPrefixes",
      IntegerValue(boost::lexical_cast<uint32_t>(nprefix)));
    consumerRegisterContentHelper.SetAttribute("RegRate",
  ...
  //Asking for route in controller
  ndn::AppHelper askRoute ("ns3::ndn::CRoSNDNRouteConsumer");
  askRoute.SetAttribute("Prefix", StringValue("/"));
  askRoute.Install (nodes);
  ...
  ndn::AppHelper DataLocation ("ns3::ndn::CRoSNDNDataLocation");
  DataLocation.Install (namenode);
  ndn::AppHelper RegisterServiceDataLocation ("ns3::ndn::CRoSNDNRegisterService");
  RegisterServiceDataLocation.SetAttribute ("Prefix", StringValue("NamedData"));
  RegisterServiceDataLocation.Install (namenode);  
   ...
   std::string regrate, std::string fibsize)
{
  if (strategy == "CRoS-NDN" || strategy == "CRoS-NDN-CRP" || strategy == "CRoS-NDN-CFP")
    configureCRoSNDN (prefix, nprefix, nodes, hellorate, controllers, producers, consumers, namenode, regrate, fibsize);
...
main (int argc, char *argv[])
{
  ...
  NodeContainer namenode;
  namenode.Add (Names::Find<Node> (failnode1));
...
//  controllers.Add (nodes.Get (contr));
  ...
  controllers.Add (Names::Find<Node> (failnode2));
...
*/
  ...
  configureStrategy (prefix, nprefix, strategy, nodes,
    hellorate, controllers, producers, consumers, namenode, regrate, fibsize);
