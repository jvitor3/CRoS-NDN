...
self.regrate, "--fibsize=%s" % self.fibsize, "--seed=%s" % self.seed, "--samplei=%s" % self.samplei, "--suffix=%s" % self.suffix,]
	...
	def writedatafile (self):
		...
		f.close()		
                f = open("temp/summaryAppDelaytime.txt","w")
                f.write("%s TimeSec Strategy Delay\n" % self.varx)
                f.close()
                f = open("temp/summaryAppDelayVar.txt","w")
                f.write("%s Strategy Delay\n" % self.varx)
                f.close()
                f = open("temp/summaryAppDelaySumtime.txt","w")
		f.write("%s Strategy TimeSec Delay sd se ci\n" % self.varx)
                f.close()
                f = open("temp/summaryStrategyAppDelayXaxis.txt","w")
                f.write("Strategy %s N Delay sd se ci\n" % self.varx)
                f.close()
	...
	def processdata1 (self):
		...
		for item in self.varset:
			...
			while (run < self.runs):
				...
				subprocess.call ("./graphs/summaryRoundAppDelay.r", shell=True)
				run = run + 1
		subprocess.call ("./graphs/summaryStrategy.r", shell=True)
		subprocess.call ("./graphs/summaryStrategyTime.r", shell=True)
		subprocess.call ("./graphs/summaryStrategyDelay.r", shell=True)
		subprocess.call ("./graphs/summaryStrategyTimeDelay.r", shell=True)
		subprocess.call ("./graphs/summaryStrategyAppDelay.r", shell=True)
                #subprocess.call ("./graphs/summaryStrategy.r", shell=True)
...
try:
	...
	topologies = ["4755.txt", "topo-3-way.txt", "topo-tree-25-node.txt", "topo-122-nodes.txt", "1221.txt", "1755.txt", "3257.txt",]
	...
	topoparameters.script = "revista1-topob"
	topoparameters.stratset = stratset5
	...
	nprefixparam.varset = [2, 20, 200, 2000, 20000]
	...
	fiblimparam2.varset = [125, 250, 500, 1000, 2000,]	
	fiblimparam2.nprefix = 2000
	fiblimparam2.rate = 1
	fiblimparam2.consprefs = 2000	
	...
	crateparam.varset = [1, 2, 4, 8, 16, 32, 64]
	...
	cachesizeexperiment3.run()
        ...
        cachesizeparam3r20 = copy.deepcopy(cachesizeparam3)
        cachesizeparam3r20.name = "cache-size3-rate20"
        cachesizeparam3r20.rate = 20
        cachesizeexperiment3r20 = CacheSize(parameters = cachesizeparam3r20)
        cachesizeexperiment3r20.run()

        cachesizeparam3r100 = copy.deepcopy(cachesizeparam3)
        cachesizeparam3r100.name = "cache-size3-rate100"
        cachesizeparam3r100.rate = 100
        cachesizeexperiment3r100 = CacheSize(parameters = cachesizeparam3r100)
        cachesizeexperiment3r100.run()
