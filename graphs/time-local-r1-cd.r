...
output3 = summarySE (data, measurevar=paste(yvar,sep = ""), groupvars=c("Strategy",paste(varx),"TimeSec") )

output2 = subset (output, (Strategy != 'Omniscient'))

pd <- position_dodge(.8)
g.a <- ggplot (output3, aes (x=TimeSec, y=eval(yvar), linetype=eval(varx2), shape=eval(varx2)))+ #,color=eval(varx2))) +
  geom_point (size=5) +
  geom_line (size=1) +
