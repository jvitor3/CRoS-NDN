...
g.all <- ggplot (output2, aes(x=eval(varx2), y=ise, fill=Strategy)) +
  ...
  scale_fill_grey() +
  theme_bw() +
  theme(legend.position="none", text = element_text(family="Times", size = 16), legend.title=element_blank(),     
  axis.text = element_text(family="Times", size = 16)) +
png (paste("./temp/NScriptsErrorXaxis.png",sep=""), width=600, height=500)
print (g.all)
dev.off ()
