...
g.all <- ggplot (output2, aes(x=eval(varx2), y=ise, fill=Strategy)) +
  ...
  geom_errorbar (aes(ymin=ise-ci,ymax=ise+ci), width=0.8, position=position_dodge(.9), size=1) +  
  ylab (yl) +
  xlab (xl) +
  scale_fill_grey() +
  theme_bw() +
...
png (paste("./temp/Mobility-Moves-r2.png",sep=""), width=500, height=500)
print (g.all)
dev.off ()
