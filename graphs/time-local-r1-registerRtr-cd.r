...
g.a <- ggplot (output1, aes (x=TimeSec, y=eval(yvar), linetype=eval(varx2), shape=eval(varx2)))+ #,color=eval(varx2))) +
  geom_point (size=5) +
  geom_line (size=1) +
  geom_errorbar (aes(ymin=(eval(yvar)-ci),ymax=(eval(yvar)+ci)), width=4, size=1) +  
  ylab ("Controller\nDiscovery\n(Interests/s)") +
  theme_bw() +
...
#  scale_shape(solid=FALSE)+
  theme(plot.margin = unit(c(6,2,-32,9),units="points"),legend.position="none", text = element_text(family="Times", size=16), legend.title = element_text(family="Times", size=16),
...
g.b <- ggplot (output2, aes (x=TimeSec, y=eval(yvar1), linetype=eval(varx2), shape=eval(varx2)))+ #,color=eval(varx2))) +
  geom_point (size=5) +
  geom_line (size=1) +
  geom_errorbar (aes(ymin=(eval(yvar1)-ci),ymax=(eval(yvar1)+ci)), width=4, size=1) +  
  ylab ("Register\nRouter\n(Interests/s)") +
  theme_bw() +
  scale_linetype_discrete(name  = xl) +
#  scale_color_discrete(name  = xl) +
  scale_shape_discrete(name  = xl, solid=FALSE) +
#  scale_shape(solid=FALSE)+
  theme(plot.margin = unit(c(2,2,-32,9),units="points"),legend.position="none", text = element_text(family="Times", size=16), legend.title = element_text(family="Times", size=16),
  axis.text = element_text(family="Times", size = 16), legend.text = element_text(family="Times", size = 16))  
...
#  scale_color_discrete(name  = xl) +
  scale_shape_discrete(name  = xl, solid=FALSE) +
#  scale_shape(solid=FALSE)+
  theme(plot.margin = unit(c(2,2,1,1),units="points"),legend.position="bottom", text = element_text(family="Times", size=16), legend.title = element_text(family="Times", size=16),
  axis.text = element_text(family="Times", size = 16), legend.text = element_text(family="Times", size = 16))
...
#g <- arrangeGrob(g.a, g.b, g.c, heights = c(0.9/5, 0.9/5, 3.2/5), nrow=3) #generates g
#ggsave(file="temp/cirate.png", g) #saves g
