...
#  geom_point (size=5) +
  geom_line (size=1) +
#  geom_errorbar (aes(ymin=(RouteRequests-ci)/samplei,ymax=(RouteRequests+ci)/samplei), width=2, size=1) +  
...
#  scale_shape(solid=FALSE)+
  theme(plot.margin = unit(c(10,2,-42,19),units="points"),legend.position="none", text = element_text(family="Times", size = lttsize), legend.title = element_text(family="Times", size = lttsize),
...
#  geom_point (size=5) +
  geom_line (size=1) +
#  geom_errorbar (aes(ymin=ise-ci,ymax=ise+ci), width=2, size=1) +  
  ylab (yl) +
  xlab (xlb) +
  theme_bw() +
  scale_linetype_discrete(name  = xl) +
#  scale_color_discrete(name  = xl) +
  scale_shape_discrete(name  = xl, solid=FALSE) +
#  scale_shape(solid=FALSE)+
  theme(plot.margin = unit(c(0,2,1,1),units="points"),legend.position="none", text = element_text(family="Times", size = lttsize), legend.title = element_text(family="Times", size = lttsize),
  axis.text = element_text(family="Times", size = lttsize), legend.text = element_text(family="Times", size = lttsize))
  
#merge all three plots within one grid
#png("temp/roaming.png", width=800, height=800)
png("temp/roamingb.png", width=500, height=500)
grid.arrange(g.a, g.b,heights = c(2/5,3/5))  
dev.off()
