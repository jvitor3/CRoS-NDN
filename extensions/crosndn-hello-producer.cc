...
NS_LOG_COMPONENT_DEFINE ("ndn.CRoSNDNHelloProducer");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (CRoSNDNHelloProducer);
   
TypeId
CRoSNDNHelloProducer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::CRoSNDNHelloProducer")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<CRoSNDNHelloProducer> ()
    .AddAttribute ("Prefix","Prefix, for which producer has the data",
                   StringValue ("/"),
                   MakeNameAccessor (&CRoSNDNHelloProducer::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("Postfix", "Postfix that is added to the output data (e.g., for adding producer-uniqueness)",
                   StringValue ("/"),
                   MakeNameAccessor (&CRoSNDNHelloProducer::m_postfix),
                   MakeNameChecker ())
    .AddAttribute ("PayloadSize", "Virtual payload size for Content packets",
                   UintegerValue (1024),
                   MakeUintegerAccessor (&CRoSNDNHelloProducer::m_virtualPayloadSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&CRoSNDNHelloProducer::m_freshness),
                   MakeTimeChecker ())
    .AddAttribute ("Signature", "Fake signature, 0 valid signature (default), other values application-specific",
                   UintegerValue (0),
                   MakeUintegerAccessor (&CRoSNDNHelloProducer::m_signature),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("KeyLocator", "Name to be used for key locator.  If root, then key locator is not used",
                   NameValue (),
                   MakeNameAccessor (&CRoSNDNHelloProducer::m_keyLocator),
                   MakeNameChecker ())
    .AddAttribute ("Frequency", "Frequency of interest packets",
                   StringValue ("1.0"),
                   MakeDoubleAccessor (&CRoSNDNHelloProducer::m_frequency),
...
CRoSNDNHelloProducer::CRoSNDNHelloProducer ()
...
void
CRoSNDNHelloProducer::StartApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
CRoSNDNHelloProducer::RefreshFib ()
{
	//m_prefix = Name("/hello");
  ...
  //Simulator::Schedule (Seconds (0.3), &CRoSNDNHelloProducer::RefreshFib, this);
  
}

void
CRoSNDNHelloProducer::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (GetNode ()->GetObject<Fib> () != 0);
  App::StopApplication ();
...
void
CRoSNDNHelloProducer::OnInterest (Ptr<const Interest> interest)
{
  ...
  if (intname.get(0).toUri() == "hello")
  {
	  ...
	  */
	  ...
	  m_events[consid] = Simulator::Schedule ( Seconds((2.5 / m_frequency)),&CRoSNDNHelloProducer::OnTimeout, this, consid);
...
void
CRoSNDNHelloProducer::OnTimeout (uint32_t nodeid)
{
...
void
CRoSNDNHelloProducer::SendInterest (Ptr<Name> prefixname)
{
...
void
CRoSNDNHelloProducer::OnData (Ptr<const ndn::Data> contentObject)
{
    ...
    sz > 0 && contentname.get(2).toUri() == "routeFrom")
  {
    ...
    if (nameaux->size() >= 1 && nameaux->getPostfix(1,0).toUri () != "/")
    {
	  ...
	  m_routeInstallTimeOut[seqn] = Simulator::Schedule (m_rtt->RetransmitTimeout (), 
        &CRoSNDNHelloProducer::OnInstallRouteTimeout, this, contentname.getPrefix (sz-6,4), seqn, nameroute->get(1).toNumber());
...
void
CRoSNDNHelloProducer::SearchController ()
{
  NS_LOG_FUNCTION_NOARGS ();
// Interest: /controller/seq
  ...
  //if ((!(m_fw->GetIsThereController ())))
  m_discoveryctl = Simulator::Schedule (m_rtt->RetransmitTimeout (),&CRoSNDNHelloProducer::OnNoController, this, interestName);
}

void
CRoSNDNHelloProducer::RegisterInController ()
{
  NS_LOG_FUNCTION_NOARGS ();
  Simulator::Cancel (m_regctl);
...
//    m_appstate = (GetNode()->GetObject<ns3::ndn::fw::CRoSNDNStrategy> ())->GetState ();
    ...
    if (ctr->size() > 1 && ctr->get(1).toNumber () != GetNode()->GetId())
      m_regctl = Simulator::Schedule (m_rtt->RetransmitTimeout (),&CRoSNDNHelloProducer::OnNoRegisterResponse, this, regname);
    SendInterest (regname);
  }
}

void
CRoSNDNHelloProducer::OnNoController(Ptr<Name> interest)
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  if ((!(m_fw->GetIsThereController ())))
    m_discoveryctl = Simulator::Schedule (m_rtt->RetransmitTimeout (),&CRoSNDNHelloProducer::SearchController, this);//, interest);
}
  
void
CRoSNDNHelloProducer::OnNoRegisterResponse(Ptr<Name> interest)
{
  NS_LOG_FUNCTION_NOARGS ();
...
bool
CRoSNDNHelloProducer::AskRoute (Ptr<const Interest> interest)
{
    ...
    contr->size () > 0)
  {
    ...
    //{
      ...
      m_routeInstallTimeOut[seqn] = Simulator::Schedule (Seconds(1.0), 
        &CRoSNDNHelloProducer::OnInstallRouteTimeout, this, pref, seqn, nameprefix->get(1).toNumber());
    ...
    (contr->size () > 1))
  {
    ...
    if (m_pendingroutes.find(pref) == m_pendingroutes.end())
    {
	  ...
	  m_askrouteTimeOut[m_seqi] = Simulator::Schedule ((m_rtt->RetransmitTimeout ()),
        &CRoSNDNHelloProducer::OnAskRouteTimeout, this, pref,namex.get(sz-1).toNumber());
	  //m_askrouteTimeOut[m_seqi] = Simulator::Schedule (Seconds(m_rtt->RetransmitTimeout ().ToInteger(Time::S) * 2),
      //  &CRoSNDNHelloProducer::OnAskRouteTimeout, this, pref,namex.get(sz-1).toNumber(), true, 0);	
		//.ToInteger(Time::S) * 2
...
void
CRoSNDNHelloProducer::OnAskRouteTimeout (Name &pref, uint32_t seqn)
{  
...
void
CRoSNDNHelloProducer::OnInstallRouteTimeout (Name &pref, uint32_t seqn, uint32_t nodeid)
{  
...
Name
CRoSNDNHelloProducer::FormatRouteString (Name prefix, uint32_t nodeid)
{
	return prefix.getPrefix (prefix.size()-4,4);
}
