...
NS_LOG_COMPONENT_DEFINE ("ndn.SDNDNGiveRoute");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (SDNDNGiveRoute);
   
TypeId
SDNDNGiveRoute::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::SDNDNGiveRoute")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<SDNDNGiveRoute> ()
    .AddAttribute ("Prefix","Prefix, for which producer has the data",
                   StringValue ("/controllerX"),
                   MakeNameAccessor (&SDNDNGiveRoute::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("Postfix", "Postfix that is added to the output data (e.g., for adding producer-uniqueness)",
                   StringValue ("/"),
                   MakeNameAccessor (&SDNDNGiveRoute::m_postfix),
                   MakeNameChecker ())
    .AddAttribute ("PayloadSize", "Virtual payload size for Content packets",
                   UintegerValue (1024),
                   MakeUintegerAccessor (&SDNDNGiveRoute::m_virtualPayloadSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0.01)),
                   MakeTimeAccessor (&SDNDNGiveRoute::m_freshness),
                   MakeTimeChecker ())
    .AddAttribute ("Signature", "Fake signature, 0 valid signature (default), other values application-specific",
                   UintegerValue (0),
                   MakeUintegerAccessor (&SDNDNGiveRoute::m_signature),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("KeyLocator", "Name to be used for key locator.  If root, then key locator is not used",
                   NameValue (),
                   MakeNameAccessor (&SDNDNGiveRoute::m_keyLocator),
                   MakeNameChecker ())   
    ;
  return tid;
}
...
void
SDNDNGiveRoute::StartApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
SDNDNGiveRoute::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (GetNode ()->GetObject<Fib> () != 0);
  App::StopApplication ();
...
void
SDNDNGiveRoute::OnInterest (Ptr<const Interest> interest)
{
...
//  const Name answer = dataName->getSubName (4, size-5);  
  ...
  if (path->size() == 0 and time.ToDouble (Time::S) > 2000)
  {
    NS_LOG_UNCOND ("Time: " << time << ", No route for name: " << answer); 
  }
  */
