...
NS_OBJECT_ENSURE_REGISTERED (OSPFLikeStrategy);

LogComponent OSPFLikeStrategy::g_log = LogComponent (OSPFLikeStrategy::GetLogName ().c_str ());

std::string
OSPFLikeStrategy::GetLogName ()
{
  return super::GetLogName ()+".OSPFLikeStrategy";
}


TypeId
OSPFLikeStrategy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::OSPFLikeStrategy")
    .SetGroupName ("Ndn")
    .SetParent <super> ()
    .AddConstructor <OSPFLikeStrategy> ()
    ;
  return tid;
}

OSPFLikeStrategy::OSPFLikeStrategy ()
{
}

void
OSPFLikeStrategy::WillEraseTimedOutPendingInterest (Ptr<pit::Entry> pitEntry)
{
...
bool
OSPFLikeStrategy::DoPropagateInterest (Ptr<Face> inFace,
...
//based on Interests from app flooding prefixes starting with floodingprefix
...
// Interest /floodingprefix/prefix
        ...
        (nameWithSequence->get(0).toUri() == "floodingprefix"))
  {
    ...
    {
	  AddFibEntry (neighbor, inFace, hopCountTag.Get());
