...
NS_LOG_COMPONENT_DEFINE ("ndn.ConsumerCbrRand3");

namespace ns3 {
namespace ndn {
    
NS_OBJECT_ENSURE_REGISTERED (ConsumerCbrRand3);
    
TypeId
ConsumerCbrRand3::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::ConsumerCbrRand3")
    .SetGroupName ("Ndn")
    .SetParent<Consumer> ()
    .AddConstructor<ConsumerCbrRand3> ()

    .AddAttribute ("Frequency", "Frequency of interest packets",
                   StringValue ("1.0"),
                   MakeDoubleAccessor (&ConsumerCbrRand3::m_frequency),
                   MakeDoubleChecker<double> ())
    
    .AddAttribute ("Randomize", "Type of send time randomization: none (default), uniform, exponential",
                   StringValue ("none"),
                   MakeStringAccessor (&ConsumerCbrRand3::SetRandomize, &ConsumerCbrRand3::GetRandomize),
                   MakeStringChecker ())

    .AddAttribute ("MaxSeq",
                   "Maximum sequence number to request",
                   IntegerValue (std::numeric_limits<uint32_t>::max ()),
                   MakeIntegerAccessor (&ConsumerCbrRand3::m_seqMax),
                   MakeIntegerChecker<uint32_t> ())

    .AddAttribute ("NumberOfPrefixes",
                   "Number of distinct prefixes",
                   StringValue ("1"),
                   MakeIntegerAccessor (&ConsumerCbrRand3::m_nprefixes),
                   MakeIntegerChecker<uint32_t> ())

    .AddAttribute ("NumberOfNodes",
                   "Number of nodes",
                   StringValue ("1"),
                   MakeIntegerAccessor (&ConsumerCbrRand3::m_nnodes),
                   MakeIntegerChecker<uint32_t> ())

    .AddAttribute ("PercentOfNewPrefix",
                   "Fraction of interests for new prefix",
                   StringValue ("100"),
                   MakeIntegerAccessor (&ConsumerCbrRand3::m_newprefixes),
                   MakeIntegerChecker<uint32_t> ())
...
ConsumerCbrRand3::ConsumerCbrRand3 ()
  : m_frequency (1.0)
  , m_firstTime (true)
  , m_random (0)
  , m_newpseq (0)
...
void
ConsumerCbrRand3::StartApplication ()
{
...
void
ConsumerCbrRand3::ScheduleNextPacket ()
{
  if (m_firstTime)
    {
      m_sendEvent = Simulator::Schedule (Seconds (0.0),
                                         &ConsumerCbrRand3::SendPacket, this);
      m_firstTime = false;
    }
...
void
ConsumerCbrRand3::SetRandomize (const std::string &value)
{
  if (m_random)
    delete m_random;
...
std::string
ConsumerCbrRand3::GetRandomize () const
{
  return m_randomType;
}
...
void
ConsumerCbrRand3::OnData (Ptr<const Data> data)
{
...
void
ConsumerCbrRand3::OnTimeout (uint32_t sequenceNumber)
{
  ...
  if (m_seqcount[sequenceNumber] > 10)
  {
    ScheduleNextPacket ();
