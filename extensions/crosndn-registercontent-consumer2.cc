...
#include "crosndn-registercontent-consumer2.h"
#include "ns3/ptr.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.CRoSNDNRegisterContentConsumer2");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (CRoSNDNRegisterContentConsumer2);

// register NS-3 type
TypeId
CRoSNDNRegisterContentConsumer2::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::ndn::CRoSNDNRegisterContentConsumer2")
    .SetGroupName ("Ndn")
    .SetParent<ndn::App> ()
    .AddConstructor<CRoSNDNRegisterContentConsumer2> ()
    .AddAttribute ("Prefix", "Requested name root",
                   StringValue ("/registeringprefix"),
                   MakeStringAccessor (&CRoSNDNRegisterContentConsumer2::m_name),
                   MakeStringChecker ())
    .AddAttribute ("NPrefixes", "Number of derived names to register",
                   IntegerValue (1),
                   MakeIntegerAccessor(&CRoSNDNRegisterContentConsumer2::m_nprefixes),
                   MakeIntegerChecker<uint32_t>())
    .AddAttribute ("RegRate", "Prefix register rate",
                   IntegerValue (1),
                   MakeIntegerAccessor(&CRoSNDNRegisterContentConsumer2::m_regrate),
                   MakeIntegerChecker<uint32_t>())                    
    .AddAttribute ("Frequency", "Frequency of interest packets",
                   StringValue ("1.0"),
                   MakeDoubleAccessor (&CRoSNDNRegisterContentConsumer2::m_frequency),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("NewLocation", "0 indicates parallel producers, 1 indicates one producer changing its location",
                   IntegerValue (0),
                   MakeIntegerAccessor(&CRoSNDNRegisterContentConsumer2::m_newlocation),
                   MakeIntegerChecker<uint32_t>())				   
    ;
  return tid;
}

CRoSNDNRegisterContentConsumer2::CRoSNDNRegisterContentConsumer2 ()
  : m_isRunning (false)
{
...
void
CRoSNDNRegisterContentConsumer2::StartApplication ()
{
  // initialize ndn::App
  ...
  Simulator::ScheduleNow (&CRoSNDNRegisterContentConsumer2::BatchInterests, this);
}

// Processing when application is stopped
void
CRoSNDNRegisterContentConsumer2::StopApplication ()
{
  m_isRunning = false;
  // cleanup ndn::App
...
void
CRoSNDNRegisterContentConsumer2::BatchInterests ()
{
  ...
  while (controllerx->size() > 0 && m_nprefixes > m_seq && count < m_regrate && m_count < m_regrate)
  {
    ...
    m_events[namesent] = Simulator::Schedule ( m_rtt->RetransmitTimeout (),
      &CRoSNDNRegisterContentConsumer2::OnTimeout, this, nameprefix);    
    SendInterest (nameprefix);     
    m_count++;
    count++;
    m_seq++;
  }
  Simulator::Schedule (Seconds (1.0), &CRoSNDNRegisterContentConsumer2::BatchInterests, this);
}


void
CRoSNDNRegisterContentConsumer2::SendInterest (Ptr<Name> prefixname)
{
...
void
CRoSNDNRegisterContentConsumer2::OnData (Ptr<const ndn::Data> contentObject)
{
...
void
CRoSNDNRegisterContentConsumer2::OnTimeout(Ptr<Name> interest)
{
  ...
  m_events[interest->getSubName()] = Simulator::Schedule ( m_rtt->RetransmitTimeout (),
    &CRoSNDNRegisterContentConsumer2::OnTimeout, this, interest);
