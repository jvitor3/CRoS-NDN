...
NS_OBJECT_ENSURE_REGISTERED (ARPLikeStrategy);

LogComponent ARPLikeStrategy::g_log = LogComponent (ARPLikeStrategy::GetLogName ().c_str ());

std::string
ARPLikeStrategy::GetLogName ()
{
  return super::GetLogName ()+".ARPLikeStrategy";
}


TypeId
ARPLikeStrategy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::fw::ARPLikeStrategy")
    .SetGroupName ("Ndn")
    .SetParent <super> ()
    .AddConstructor <ARPLikeStrategy> ()
    ;
  return tid;
}

ARPLikeStrategy::ARPLikeStrategy ()
{
}

void
ARPLikeStrategy::WillEraseTimedOutPendingInterest (Ptr<pit::Entry> pitEntry)
{
...
bool
ARPLikeStrategy::DoPropagateInterest (Ptr<Face> inFace,
...
void
ARPLikeStrategy::SatisfyPendingInterest (Ptr<Face> inFace,
                                            Ptr<const Data> data,
                                            Ptr<pit::Entry> pitEntry)
{
  if (pitEntry->GetFibEntry ()->GetPrefix () == Name("/"))
