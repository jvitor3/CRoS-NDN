...
NS_LOG_COMPONENT_DEFINE ("ndn.SDNDNAskRoute");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (SDNDNAskRoute);
    
TypeId
SDNDNAskRoute::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::SDNDNAskRoute")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<SDNDNAskRoute> ()
    .AddAttribute ("Prefix","Prefix, for which producer has the data",
                   StringValue ("/"),
                   MakeNameAccessor (&SDNDNAskRoute::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("PayloadSize", "Virtual payload size for Content packets",
                   UintegerValue (1024),
                   MakeUintegerAccessor(&SDNDNAskRoute::m_virtualPayloadSize),
                   MakeUintegerChecker<uint32_t>())
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&SDNDNAskRoute::m_freshness),
                   MakeTimeChecker ())
    ;
  return tid;
}
    
SDNDNAskRoute::SDNDNAskRoute ()
{
...
void
SDNDNAskRoute::StartApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
SDNDNAskRoute::RefreshFib ()
{
  (GetNode()->GetObject<ns3::ndn::fw::BestRouteSDNDNController> ())
    ->AddFibEntry (m_prefix, m_face, 10);
  Simulator::Schedule (Seconds (0.3), &SDNDNAskRoute::RefreshFib, this);
}

void
SDNDNAskRoute::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (GetNode ()->GetObject<Fib> () != 0);
  App::StopApplication ();
...
void
SDNDNAskRoute::OnTimeout (Name &pref, uint32_t sequenceNumber, bool isAskRoute)
{
  if (m_pendingroutes.find(pref) != m_pendingroutes.end())
  {
    ...
    if (isAskRoute)
    {
      ...
      (GetNode()->GetObject<ns3::ndn::fw::BestRouteSDNDNController> ())->SetRegisteredRouter (false);
    }
  }
}

bool
SDNDNAskRoute::AskRoute (Ptr<const Interest> interest)
{
  Name namex = interest->GetName ();
  NS_LOG_DEBUG ("Node asking route for name: " << namex);
  uint32_t sz = namex.size ();
  FwHopCountTag hopCountTag;
  Ptr<Name> contr = (GetNode()->GetObject<ns3::ndn::fw::BestRouteSDNDNController> ())->GetController ();
  if (contr->size () == 0)
  {
    ...
    contr->size () > 0)
  {
    ...
    if (m_pendingroutes.find(pref) == m_pendingroutes.end())
    {
      ...
      Simulator::Schedule (Seconds(1.0), 
        &SDNDNAskRoute::OnTimeout, this, pref, namex.get(sz-1).toNumber(), false);
  ...
  else if ((namex.get(0).toUri() != "controllerX") &&
    ...
    (contr->size () > 1))
  {
    ...
    if (m_pendingroutes.find(pref) == m_pendingroutes.end())
    {
      ...
      m_sendEvent = Simulator::Schedule ((m_rtt->RetransmitTimeout ()),
        &SDNDNAskRoute::OnTimeout, this, pref,namex.get(sz-1).toNumber(), true);
...
void
SDNDNAskRoute::OnInterest (Ptr<const Interest> interest)
{
...
void
SDNDNAskRoute::OnData (Ptr<const ndn::Data> contentObject)
{
  ...
  //prefix: /controllerX/controllerId/routeFrom/sourceRouterID/wantedprefix/seq
  if (contentname.get(0).toUri() == "controllerX" &&
