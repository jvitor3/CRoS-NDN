...
#include "ndn-nlsr-v2-askroute.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.NLSRV2AskRoute");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (NLSRV2AskRoute);
    
TypeId
NLSRV2AskRoute::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::NLSRV2AskRoute")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<NLSRV2AskRoute> ()
    .AddAttribute ("Prefix","Prefix, for which producer has the data",
                   StringValue ("/"),
                   MakeNameAccessor (&NLSRV2AskRoute::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("PayloadSize", "Virtual payload size for Content packets",
                   UintegerValue (1024),
                   MakeUintegerAccessor(&NLSRV2AskRoute::m_virtualPayloadSize),
                   MakeUintegerChecker<uint32_t>())
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&NLSRV2AskRoute::m_freshness),
                   MakeTimeChecker ())
    ;
  return tid;
}
    
NLSRV2AskRoute::NLSRV2AskRoute ()
{
...
void
NLSRV2AskRoute::StartApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
NLSRV2AskRoute::RefreshFib ()
{
  (GetNode()->GetObject<ns3::ndn::fw::BestRouteNLSRV2> ())
    ->AddFibEntry (m_prefix, m_face, 10);  
  Simulator::Schedule (Seconds (0.3), &NLSRV2AskRoute::RefreshFib, this);
}

void
NLSRV2AskRoute::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (GetNode ()->GetObject<Fib> () != 0);
  App::StopApplication ();
...
bool
NLSRV2AskRoute::AskRoute (Ptr<const Interest> interest)
{
  ...
  std::set<uint32_t> candidates = (GetNode()->
    GetObject<ns3::ndn::fw::BestRouteNLSRV2> ())->GetRegisteredContentNodes (answer);
    ...
    it1 != candidates.end(); it1++)
  {
    ...
    Ptr<Name> temp = (GetNode()->
      GetObject<ns3::ndn::fw::BestRouteNLSRV2> ())->GetRoutes (node1,*(it1));
    if (path->size () == 0)
      path = temp;
  ...
  if (path->size() > 0)
  {
    ...
    if (fibentry != 0)
    {
      ...
      (GetNode()->
      GetObject<ns3::ndn::fw::BestRouteNLSRV2> ())->AddFibEntry (wantedprefix, facex, 0);
      return true;
    }
  }
  else
...
void
NLSRV2AskRoute::OnInterest (Ptr<const Interest> interest)
{
...
void
NLSRV2AskRoute::OnData (Ptr<const ndn::Data> contentObject)
{
