...
#include "nlsr-hashes-producer.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.NLSRHashesProducer");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (NLSRHashesProducer);
   
TypeId
NLSRHashesProducer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::NLSRHashesProducer")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<NLSRHashesProducer> ()
    .AddAttribute ("Prefix","Prefix, for which producer has the data",
                   StringValue ("/"),
                   MakeNameAccessor (&NLSRHashesProducer::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("Postfix", "Postfix that is added to the output data (e.g., for adding producer-uniqueness)",
                   StringValue ("/"),
                   MakeNameAccessor (&NLSRHashesProducer::m_postfix),
                   MakeNameChecker ())
    .AddAttribute ("PayloadSize", "Virtual payload size for Content packets",
                   UintegerValue (1024),
                   MakeUintegerAccessor (&NLSRHashesProducer::m_virtualPayloadSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0.15)),
                   MakeTimeAccessor (&NLSRHashesProducer::m_freshness),
                   MakeTimeChecker ())
    .AddAttribute ("Signature", "Fake signature, 0 valid signature (default), other values application-specific",
                   UintegerValue (0),
                   MakeUintegerAccessor (&NLSRHashesProducer::m_signature),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("KeyLocator", "Name to be used for key locator.  If root, then key locator is not used",
                   NameValue (),
                   MakeNameAccessor (&NLSRHashesProducer::m_keyLocator),
                   MakeNameChecker ())
    ;
  return tid;
}
...
NLSRHashesProducer::NLSRHashesProducer ()
{
...
void
NLSRHashesProducer::StartApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
NLSRHashesProducer::RefreshFib ()
{
  m_fw->AddFibEntry (m_prefix, m_face, 0);
  Simulator::Schedule (Seconds (0.1), &NLSRHashesProducer::RefreshFib, this);
}

void
NLSRHashesProducer::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (GetNode ()->GetObject<Fib> () != 0);
  App::StopApplication ();
...
void
NLSRHashesProducer::OnInterest (Ptr<const Interest> interest)
{
  ...
  Name contenthashes = m_fw->GetBranchComponents (prefix);
  uint32_t bytes = 0;
