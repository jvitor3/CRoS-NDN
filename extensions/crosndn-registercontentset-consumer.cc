...
#include "crosndn-registercontentset-consumer.h"
#include "ns3/ptr.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.CRoSNDNRegisterContentSetConsumer");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (CRoSNDNRegisterContentSetConsumer);

// register NS-3 type
TypeId
CRoSNDNRegisterContentSetConsumer::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::ndn::CRoSNDNRegisterContentSetConsumer")
    .SetGroupName ("Ndn")
    .SetParent<ndn::App> ()
    .AddConstructor<CRoSNDNRegisterContentSetConsumer> ()
    .AddAttribute ("Prefix", "Requested name root",
                   StringValue ("/registeringprefix"),
                   MakeStringAccessor (&CRoSNDNRegisterContentSetConsumer::m_name),
                   MakeStringChecker ())
    .AddAttribute ("NPrefixes", "Number of derived names to register",
                   IntegerValue (1),
                   MakeIntegerAccessor(&CRoSNDNRegisterContentSetConsumer::m_nprefixes),
                   MakeIntegerChecker<uint32_t>())
    .AddAttribute ("RegRate", "Prefix register rate",
                   IntegerValue (1),
                   MakeIntegerAccessor(&CRoSNDNRegisterContentSetConsumer::m_regrate),
                   MakeIntegerChecker<uint32_t>())                    
    .AddAttribute ("Frequency", "Frequency of interest packets",
                   StringValue ("1.0"),
                   MakeDoubleAccessor (&CRoSNDNRegisterContentSetConsumer::m_frequency),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("NewLocation", "0 indicates parallel producers, 1 indicates one producer changing its location",
                   IntegerValue (0),
                   MakeIntegerAccessor(&CRoSNDNRegisterContentSetConsumer::m_newlocation),
                   MakeIntegerChecker<uint32_t>())				   
    ;
  return tid;
}

CRoSNDNRegisterContentSetConsumer::CRoSNDNRegisterContentSetConsumer ()
  : m_isRunning (false)
{
...
void
CRoSNDNRegisterContentSetConsumer::StartApplication ()
{
  // initialize ndn::App
  ...
  Simulator::ScheduleNow (&CRoSNDNRegisterContentSetConsumer::BatchInterests, this);
}

// Processing when application is stopped
void
CRoSNDNRegisterContentSetConsumer::StopApplication ()
{
  m_isRunning = false;
  // cleanup ndn::App
...
void
CRoSNDNRegisterContentSetConsumer::BatchInterests ()
{
  ...
  {
	  Simulator::Schedule (Seconds (1.0), &CRoSNDNRegisterContentSetConsumer::BatchInterests, this);
  }
  else if (m_nprefixes > m_seq)
  {
...
void
CRoSNDNRegisterContentSetConsumer::SendInterest (Ptr<Name> prefixname)
{
...
void
CRoSNDNRegisterContentSetConsumer::OnData (Ptr<const ndn::Data> contentObject)
{
...
void
CRoSNDNRegisterContentSetConsumer::OnTimeout(Ptr<Name> interest)
{
  ...
  m_events[interest->getSubName()] = Simulator::Schedule ( m_rtt->RetransmitTimeout (),
    &CRoSNDNRegisterContentSetConsumer::OnTimeout, this, interest);
