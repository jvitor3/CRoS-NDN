...
#include "crosndn-tunnel-route-consumer.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("ndn.CRoSNDNTunnelRouteConsumer");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED (CRoSNDNTunnelRouteConsumer);
    
TypeId
CRoSNDNTunnelRouteConsumer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ndn::CRoSNDNTunnelRouteConsumer")
    .SetGroupName ("Ndn")
    .SetParent<App> ()
    .AddConstructor<CRoSNDNTunnelRouteConsumer> ()
    .AddAttribute ("Prefix","Prefix, for which producer has the data",
                   StringValue ("/"),
                   MakeNameAccessor (&CRoSNDNTunnelRouteConsumer::m_prefix),
                   MakeNameChecker ())
    .AddAttribute ("PayloadSize", "Virtual payload size for Content packets",
                   UintegerValue (1024),
                   MakeUintegerAccessor(&CRoSNDNTunnelRouteConsumer::m_virtualPayloadSize),
                   MakeUintegerChecker<uint32_t>())
    .AddAttribute ("Freshness", "Freshness of data packets, if 0, then unlimited freshness",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&CRoSNDNTunnelRouteConsumer::m_freshness),
                   MakeTimeChecker ())
    ;
  return tid;
}
    
CRoSNDNTunnelRouteConsumer::CRoSNDNTunnelRouteConsumer ()
{
...
void
CRoSNDNTunnelRouteConsumer::StartApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
CRoSNDNTunnelRouteConsumer::RefreshFib ()
{
  m_fw->AddFibEntry (m_prefix, m_face, 10);
  Simulator::Schedule (Seconds (0.3), &CRoSNDNTunnelRouteConsumer::RefreshFib, this);
}

void
CRoSNDNTunnelRouteConsumer::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_ASSERT (GetNode ()->GetObject<Fib> () != 0);
  App::StopApplication ();
...
void
CRoSNDNTunnelRouteConsumer::OnTimeout (Name &pref, uint32_t sequenceNumber, bool isAskRoute)
{
...
bool
CRoSNDNTunnelRouteConsumer::AskRoute (Ptr<const Interest> interest)
{
    ...
    contr->size () > 0)
  {
    ...
    if (m_pendingroutes.find(pref) == m_pendingroutes.end())
    {
      ...
      Simulator::Schedule (Seconds(1.0), 
        &CRoSNDNTunnelRouteConsumer::OnTimeout, this, pref, namex.get(sz-1).toNumber(), false);
	...
	(namex.get(0).toUri() != "tunnel") &&  //05may2016
    (contr->size () > 1))
  {
	//change 11april2016
    ...
    if (m_pendingroutes.find(pref) == m_pendingroutes.end())
    {
      ...
      m_sendEvent = Simulator::Schedule ((m_rtt->RetransmitTimeout ()),
        &CRoSNDNTunnelRouteConsumer::OnTimeout, this, pref,namex.get(sz-1).toNumber(), true);
...
void
CRoSNDNTunnelRouteConsumer::OnInterest (Ptr<const Interest> interest)
{
...
void
CRoSNDNTunnelRouteConsumer::OnData (Ptr<const ndn::Data> contentObject)
{
    ...
    contentname.size() > 0 && contentname.get(2).toUri() == "routeFrom")
  {
    ...
    if (nameaux->size() >= 1 && nameaux->getPostfix(1,0).toUri () != "/")
    {
	  ...
	  tunnelhop.append(nameroute->get(nameroute->size()-1));
      nameroute->append("endRoute");
      nameroute->append("routePreference");
      nameroute->appendSeqNum(1);
      nameroute->append("toPrefix");
	  nameroute->append(tunnelhop);
      nameroute->append (contentname.getPrefix (contentname.size()-5,4));
	  m_fw->AddFibEntry (contentname.getPrefix (contentname.size()-7,4), m_fw->GetTunnelFace(), 0);
	  m_fw->SetTunnelDestination(contentname.getPrefix (contentname.size()-7,4), nameroute->get(nameroute->size()-1).toNumber());
