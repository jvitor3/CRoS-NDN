...
#ifndef NLSR_STRATEGY_H
#define NLSR_STRATEGY_H
...
//  AddGraphEdges (uint32_t id1, uint32_t id2, FaceInfo faceinfo);
  ...
  void
  UnRegisterContent (const Name &name);
  ...
  Ptr<Name>
  GetRoutes2 (uint32_t sourceid, uint32_t targetid);  
  ...
  bool
  InstallRoute (Ptr<Face> inFace, Ptr<const Interest> interest);
...
#endif // NLSRStrategyS_STRATEGY_H
